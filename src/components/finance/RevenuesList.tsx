...
import { Plus, Search, Pencil, Trash2, ArrowDownLeft } from 'lucide-react';
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="dashboard-panel">
          <CardContent className="p-4">
            <div className="text-sm text-muted-foreground">Total recettes</div>
            <div className="text-2xl font-bold text-success">+{stats.total.toFixed(2)} MAD</div>
          </CardContent>
        </Card>
        <Card className="dashboard-panel">
          <CardContent className="p-4">
          ...
          <div className="flex flex-col sm:flex-row gap-2">
            ...
            <Dialog open={isAddOpen} onOpenChange={setIsAddOpen}>
              ...
              <DialogContent className="max-w-2xl">
                ...
                <PaymentForm fixedPaymentType="income" onSuccess={() => setIsAddOpen(false)} />
              </DialogContent>
            </Dialog>
          </div>
        </CardHeader>
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
            ...
            <div>
              ...
              <Input value={category} onChange={(e) => setCategory(e.target.value)} placeholder="Vente..." />
            </div>
          </div>
              ...
              filtered.map((p) => (
                <div key={p.id} className="border rounded-lg p-3 space-y-2">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <div className="text-sm font-semibold">{p.category || 'Recette'}</div>
                      <div className="text-xs text-muted-foreground">
                        {format(new Date(p.payment_date), 'dd/MM/yyyy')} • {p.entity_name || '-'}
                      </div>
                    </div>
                    <div className="text-right">
                      <div className="font-bold text-success">+{Number(p.amount).toFixed(2)} MAD</div>
                      <Badge variant={STATUS_VARIANT[p.status] || 'secondary'}>{p.status || 'completed'}</Badge>
                    </div>
                  </div>
                  <div className="flex items-center justify-end gap-1">
      ...
      <Dialog open={!!editing} onOpenChange={(open) => (!open ? setEditing(null) : undefined)}>
        <DialogContent className="max-w-2xl">
          ...
          {editing && <PaymentForm fixedPaymentType="income" payment={editing} onSuccess={() => setEditing(null)} />}
        </DialogContent>
      </Dialog>
    </div>
  );
