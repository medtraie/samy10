...
import { Route, ArrowRight, Trash2, Loader2, Target } from 'lucide-react';
...
export function TrajetsList() {
  const { data: trajets = [], isLoading: trajetsLoading } = useTrajets();
  const { data: geofences = [], isLoading: geofencesLoading } = useGPSwoxGeofences();
  const deleteTrajet = useDeleteTrajet();

  const isLoading = trajetsLoading || geofencesLoading;
    ...
    <div className="space-y-6">
      ...
      {trajets.length === 0 ? (
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {trajets.map((trajet) => {
            const isOriginGeo = trajet.origin_chantier?.name && geofenceNames.has(trajet.origin_chantier.name.toLowerCase());
            const isDestGeo = trajet.destination_chantier?.name && geofenceNames.has(trajet.destination_chantier.name.toLowerCase());

            return (
              <Card key={trajet.id} className="dashboard-panel">
                <CardHeader className="pb-2">
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <CardTitle className="text-base truncate max-w-[160px]" title={trajet.name}>{trajet.name}</CardTitle>
                    </div>
                ...
                <CardContent className="space-y-3 text-sm">
                  <div className="flex items-center gap-2 bg-muted/50 p-2 rounded-md">
                    <div className="flex-1 truncate" title={trajet.origin_chantier?.name || 'Non défini'}>
                      <span className="text-muted-foreground flex items-center gap-1 text-xs">
                        {isOriginGeo ? <Target className="w-3 h-3 text-primary" /> : null}
                        {trajet.origin_chantier?.name || 'Non défini'}
                      </span>
                    </div>
                    <ArrowRight className="w-4 h-4 text-muted-foreground flex-shrink-0" />
                    <div className="flex-1 text-right truncate" title={trajet.destination_chantier?.name || 'Non défini'}>
                      <span className="text-muted-foreground flex items-center justify-end gap-1 text-xs">
                        ...
                        {isDestGeo ? <Target className="w-3 h-3 text-primary" /> : null}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    {trajet.estimated_duration_minutes && (
                      <div>
                        <span className="text-muted-foreground text-xs block">Durée est.</span>
                  ...
                  <div className="grid grid-cols-2 gap-2 pt-2 border-t">
                    <div>
                      <span className="text-muted-foreground text-xs block">Prix/tonne</span>
                      <p className="font-semibold text-primary">{trajet.price_per_ton} MAD</p>
                    </div>
                    {trajet.price_per_trip > 0 && (
                      <div>
                        <span className="text-muted-foreground text-xs block">Prix/voyage</span>
